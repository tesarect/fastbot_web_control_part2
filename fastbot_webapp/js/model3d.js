 ...
 * - param: '/fastbot_1_robot_state_publisher:robot_description' (node:param format)
 * - fixedFrame: 'map' (requires nav2 running with AMCL)
 * - path: location.origin + location.pathname (no query params)
 * - Mesh folder: meshes/fastbot_description/onshape/new_assets/
 */
...
const model3d = (() => {
  ...
  function init(ros) {
    ...
    // TF client — 'map' frame requires nav2+AMCL running
    ...
    // UrdfClient with node:param format (key insight from reference)
